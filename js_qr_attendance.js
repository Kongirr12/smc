...
function qraProcess(code) {
  ...
  google.script.run
    .withSuccessHandler(function(res) { qraResult(res, code); })
    .withFailureHandler(function(e)   { qraResult({ status:'error', message: e.message||String(e) }, code); })
    .recordQRAttendance(code, QRA.date, APP.token);
}
