...
function loadBehaviorData() {
  ...
  google.script.run
    ...
    .getBehaviorSummary(APP.token);

  google.script.run
    .withSuccessHandler(res => { if (res.status==='success') BHV.presets = res.data; check(); })
    .withFailureHandler(() => check())
    .getBehaviorPresets(APP.token);
}
...
function loadBhvHistory() {
  ...
  google.script.run
    ...
    .withFailureHandler(err => { area.innerHTML = '<div class="empty-state">เกิดข้อผิดพลาด</div>'; })
    .getBehaviorRecords(params, APP.token);
}
...
function openBehaviorForm(preStudentId) {
  ...
  Swal.fire({
    ...
    google.script.run
      ...
      .saveBehaviorRecord(res.value, APP.token);
  });
}
...
function deleteBhvRecord(id) {
  Swal.fire({ title:'ลบรายการนี้?', icon:'warning', showCancelButton:true,
    ...
    google.script.run
      .withSuccessHandler(res => { if (res.status==='success') { showToast('success','ลบสำเร็จ'); loadBhvHistory(); loadBehaviorData(); } })
      .deleteBehaviorRecord(id, APP.token);
  });
}
